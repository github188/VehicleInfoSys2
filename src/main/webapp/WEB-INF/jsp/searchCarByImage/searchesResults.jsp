...
<div id="gg" class="easyui-layout" style="width:100%;height:100%;">
    <div id="scarches_query_win"
         style="position: relative; width: 100%; height: 100%;"></div>

    <div id="distinguishes" data-options="region:'south',title:'检索结果&ensp;&ensp;&ensp;当前检索进度：0% ',split:true" style="height:300px;">
        <div id="retrievales_results"></div>
    </div>
    <%--<div data-options="region:'east',title:'详细信息对比',split:true" style="width:45%;">
        <div id="detailed_information_original" style="float:left;margin-left: 65px"></div>
        <div id="detailed_information" style="float:right"></div>
    </div>--%>

    <div data-options="region:'center',title:'原始图片（左）与目标图片（右）'" style="background:#eee;">
        <div id="imges_original" style="width: 50%;height: 100%;float: left;"></div>
        <div id="imges" style="width: 50%;height: 100%;float: right;"></div>
    </div>
</div>

<%-- 没有任何结果时用 --%>
<script type="tmpl" id="wues_vlpr_vfm_result_original_tmpl">
...
<script type="tmpl" id="vlpres_vfm_result_original_tmpl">
<div style="position:relative;left:40px;float: left;">
     <table class="info_table" style="margin:0 auto;">
        <tr>
            <td colspan="3">原始图片详情</td>
...
<script type="tmpl" id="vlpres_vfm_result_originals_tmpl">
<div>
    <div class="box-small">
...
<script type="tmpl" id="vlpres_vfm_result_tmpl">
...
<script type="tmpl" id="vlpres_vfm_results_tmpl">
<div>
    <div class="box-small">
		<img width="100%" height="100%" src="{#imageUrl}" alt="" />
    	<div class="zoomMark" id="zoomMark">&nbsp;</div>
...
<script>
    (function ($) {
        var wues_vlpr_vfm_result_original_tmpl = $('#wues_vlpr_vfm_result_original_tmpl').html();

        var vlpr_vfm_result_original_tmpl = $('#vlpres_vfm_result_original_tmpl').html();
        var vlpr_vfm_result_originals_tmpl = $('#vlpres_vfm_result_originals_tmpl').html();

        var vlpr_vfm_result_tmpl = $('#vlpres_vfm_result_tmpl').html();
        var vlpr_vfm_results_tmpl = $('#vlpres_vfm_results_tmpl').html();

        var $bk_task_datagrid = $('#bk_task_datagrid'),
                $bk_result_datagrid = $('#retrievales_results');
        ...
        scar_loadResultDatas = function ( pageNumber, vlprVfmTaskId, vlprTaskId, serialNumber) {
            ...
            if(serialNumber){
                $.get("/serachCarByImage/originallistdetails.action",{'taskID':serialNumber},function (data){
                    ...
                    $("#detailed_information_original").append(html11);
                    $("#imges_original").append(html22);
                });
            }else if(vlprTaskId){
                $.get("/serachCarByImage/originallisttask.action",{'taskID':vlprTaskId},function (data){
                    ...
                    html11 = util.parser(wues_vlpr_vfm_result_original_tmpl, cloneRowses);
                    var html22 = util.parser(vlpr_vfm_result_originals_tmpl, cloneRowses);
                    $("#detailed_information_original").append(html11);
                    $("#imges_original").append(html22);
                });
        ...
        var showSurveillanceWindow = function (row) {
            ...
            //$("#detailed_information").append(html1);
            $("#imges").append(html2);
        }
        ...
        var refresh = function () {
            setTimeout(function () {
                if ($bk_result_datagrid.data('datagrid')) {
                    ...
                    $.post("/serachCarByImage/originallisttaskid.action",{'taskID':soucheidb},function (data){
                        var title = "检索结果&ensp;&ensp;&ensp;当前检索进度：" +  data.rows[0].progress + "% "
                        $('#distinguishes').panel('setTitle', title);
