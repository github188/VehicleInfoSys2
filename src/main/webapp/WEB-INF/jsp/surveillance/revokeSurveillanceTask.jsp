...
<div id="bk_revoke_task_datagrid"></div>
<script type="text/javascript">
$(function(){
	var tab = util.getSelectedTab();
    var datagrid=$("#bk_revoke_task_datagrid").datagrid({
        multiple: true,
            ...
            {field: 'startTime', title: '申请布控时间', width: 150, formatter: util.formateTime},
            {
             ...
             },
             {
                field: 'xxx', title: '操作', width: 180, formatter: function (v, row) {
                    ...
                    var html = '<a href="javascript:void(0)" id="' + rowId + '" class="easyui-linkbutton" data-options="iconCls:\'icon-ok\'" >审核(撤控)</a>&nbsp;&nbsp;';
                    tab.off('click', '#' + rowId).on('click', '#' + rowId, function () {
                        //弹出审核框                                     
                    	revokeSurveillanceTask(row.id,row.name,datagrid);
                    });
                    var html2 = '<a href="javascript:void(0)" id="' +'dtr'+rowId + '" class="easyui-linkbutton" data-options="iconCls:\'icon-ok\'" >详情</a>';
                    tab.off('click', '#dtr' + rowId).on('click', '#dtr' + rowId, function () {
                        //弹出详情框                                     
                    	showDeatailSurveillanceTask(row,true,revokeSurveillanceTask(row.id,row.name,datagrid));
                    })
                    return html+html2;
            }
            } 
