...
<div id="realImage_page" class="easyui-layout" fit="true" style="min-width: 800px;">
        ...
        <table class="info_area">
            <tr>
                <td>
                    <span>任务状态：</span>
                    <c:choose>
                        <c:when test="${!empty realImageTaskId}">
                            <label>识别中</label>&nbsp;&nbsp;
                            <input type="button" id="realImage_task_switch" value="停止识别"/>
                        </c:when>
                        <c:otherwise>
                            <label>未识别</label>&nbsp;&nbsp;
                            <input type="button" id="realImage_task_switch" value="开始识别"/>
                        </c:otherwise>
                    </c:choose>
                    <label style="color: red;">(请先在离线源中上传一张该监控点的画面截图用来画感兴趣区！)</label>
                </td>
            </tr>
        </table>
    </div>
    <div region="center" border="false">
        <div id="realImage_tabs" class="easyui-tabs result_details_tabs" fit="true">
            <div title="任务列表">
                <div id="realImage_task_grid" border=false>
                    <div class="toolbar">
                        <a id="realImage_remove" href="javascript:void(0)" class="easyui-linkbutton"
...
<script type="text/javascript">
    $(function () {
        ...
        var realImage_page = $('#realImage_page', tab[0]);
        var realImage_tabs = $('#realImage_tabs', tab[0]);  //实时tab页
        var realImage_task_grid = $('#realImage_task_grid', tab[0]); //实时任务列表
        var realImageUrl = $('#realImageUrl', tab[0]);
        if(${camera.status==1}) {
            util.createRemoveEvent('#realImage_remove', realImage_task_grid, 'task/delete.action');
            $('#realImage_task_switch', realImage_page).click(function () {
            var that = $(this);
            var label = that.prev();
            ...
            var startTaskCallBack = function (msg) {
                ...
                label.html('识别中');
                that.val('停止识别');
                realImage_task_grid.datagrid('load');
            };

            if (flagStr == '未识别') {
                $.post('camera/getBigUrl.action',{id:cameraId},function(d) {
                    var bigUrl = d;
                    drawLine(cameraId, bigUrl, 4, followarea, null, startTaskCallBack)
                })
            } else {
            ...
            $('#realImage_task_switch', realImage_page).attr("disabled",true);
        }

        function refresh()
        {
            if(isRefresh) return;
            realImage_task_grid.datagrid('reload');
        ...
        var newTab = function (row) {
            ...
            if (realImage_tabs.tabs('exists', title)) {
                realImage_tabs.tabs('select', title)
            } else {
                realImage_tabs.tabs('add', {
                    title: title,
                    closable: true,
        ...
        setTimeout(function () {
            //定义实时图片历史任务列表
            realImage_task_grid.datagrid({
                url: 'task/list.action?type=4&cameraId=' + cameraId,
                loadMsg: '数据载入中',
                pagination: true,
                rownumbers: true,
                fit: true,
                ...
                onLoadSuccess: function (data) {//列表加载数据成功时的回调函数
                    ...
                    if (list.length > 0) {
                        ...
                        var that = $('#realImage_task_switch', tab[0]);
                        var label = that.prev();
                        if (status == 1) {
                            label.html('识别中');
                            that.val('停止识别');
                ...
                toolbar: $('#realImage_task_grid .toolbar', tab[0]).get(0)
            });
        }, 200);
