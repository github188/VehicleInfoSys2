...
function MarkerClusterer(map, opt_markers, opt_opts) {
  ...
  function reAddMarkers_(markers) {
    ...
    // TODO 暂时暴力修改源代码解决引用聚合框架后无法屏蔽右键的bug
    setTimeout(function() {
...
function Cluster(markerClusterer) {
  ...
  this.redraw_ = function (isForce) {
    ...
    if (zoom_ >= mz || this.getTotalMarkers() === 1) {
      ...
      var totalMarkers=this.getTotalMarkers();
      if (clusterMarker_ === null) {
    	  if(totalMarkers>0){
        clusterMarker_ = new ClusterMarker_(center_, totalMarkers, markerClusterer_.getStyles_(), markerClusterer_.getGridSize_());
        ...
        clusterMarker_.redraw(true,totalMarkers);
      }
    }
  };
 ...
 * @private
 */
ClusterMarker_.prototype.redraw = function (force,totalMarkers) {
  if (!force) {
    return;
  }
  ...
  if(totalMarkers>0){
   this.div_.innerHTML=totalMarkers;
  }
};
