...
var gIconCamera = customIcon("images/IconCamera11.png");//监控点图标
var	gIconCameraInvalid = customIcon("images/IconCameraInvalid22.png");//失效监控点图标
 ...
 */
//GLabelMarker start
GLabelMarker = function(latlng, options)
{
    this.labelText = options.labelText || '';
    this.labelClass = options.labelClass || 'writeb';
    this.labelOffset = options.labelOffset || new GSize(-15, -35);
...
GLabelMarker.prototype = new GMarker(new GLatLng(0, 0));
GLabelMarker.prototype.initialize = function(map){
...
GLabelMarker.prototype.redraw = function(force){
    GMarker.prototype.redraw.call(this, this.map);
    if(!force)return;
    var latlng=this.getLatLng(),
        point = this.map.fromLatLngToDivPixel(latlng),
        z = GOverlay.getZIndex(latlng.lat());
    ...
    this.Yv.style.width = this.label.offsetWidth+'px';
}
GLabelMarker.prototype.remove = function(){
    this.label.parentNode.removeChild(this.label);
    this.label = null;
    GMarker.prototype.remove.call(this);
}
function getTextIcon(width, offset)
{
    var icon = new GIcon();
    icon.image = "";
